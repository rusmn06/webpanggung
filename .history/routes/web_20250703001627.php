...
Route::prefix('admin')
    ...
    ->name('tkw.') // Di routelist akan menjadi admin.tkw, saya asumsikan ini di dalam grup admin
    ->group(function () {
    
    // --- Rute STATIS (paling spesifik), letakkan di paling atas ---
    ...
    // --- Rute DINAMIS yang lebih spesifik ---
    // Pola /detail/{id} dan /export-excel/{id} lebih spesifik dari /{id}
    ...
    // Pola /{id}/edit lebih spesifik dari /{id}
    Route::get('/{id}/edit', [TenagaKerjaVerifController::class, 'edit'])->name('edit');
    
    // Pola /rt/{rt} juga spesifik karena diawali 'rt'
    Route::get('/rt/{rt}', [TenagaKerjaVerifController::class, 'showRtData'])->name('rtshow')->where('rt', '[0-9]+');
    
    // --- Rute untuk PROSES FORM (metode berbeda) ---
    ...
    // --- Rute DINAMIS paling UMUM, letakkan di PALING BAWAH ---
