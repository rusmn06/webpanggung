...
Route::middleware(['auth', 'role:user|admin'])->group(function () {
         ...
         for ($i = 1; $i <= 24; $i++) {
            ...
            // Di dalam grup middleware ['auth', 'role:user|admin'] (atau yang sesuai)
            Route::prefix('tenagakerja')->name('tenagakerja.')->group(function () {
                // Halaman Dashboard Kuesioner Tenaga Kerja (menampilkan daftar pengajuan user)
                Route::get('/', [UserTenagaKerjaController::class, 'index'])->name('index');

                // Halaman detail pengajuan user
                Route::get('/{id}', [UserTenagaKerjaController::class, 'show'])->name('show');

                // Route untuk listrt yang lama mungkin tidak diperlukan lagi jika fungsinya sudah digantikan oleh 'index'
                // Route::get('/listrt', [UserTenagaKerjaController::class, 'listrt'])->name('listrt');
            });
