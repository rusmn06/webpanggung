     ...
     ->group(function () {
         ...
         Route::prefix('tenagakerja')
              ...
              ->group(function () {
                  ...
                  Route::get('/rt/{rt}',        [TenagaKerjaVerifController::class, 'showRtData'])->name('showrt')
                                                             ->where('rt', '[0-9]+');
              });
...
Route::middleware(['auth', 'role:user|admin'])->group(function () {
         ...
         // Wizard Tenaga Kerja
         Route::prefix('tenagakerja')
            ->name('tkw_wizard.') // <--- DIUBAH MENJADI 'tkw_wizard.'
            ->group(function () {
                Route::get('step-1', [TenagaKerjaWizardController::class, 'showStep1'])->name('step1');
                Route::post('step-1', [TenagaKerjaWizardController::class, 'postStep1'])->name('post_step1'); // <-- DIBERI NAMA JELAS
                Route::get('step-2', [TenagaKerjaWizardController::class, 'showStep2'])->name('step2');
                Route::post('step-2', [TenagaKerjaWizardController::class, 'postStep2'])->name('post_step2'); // <-- DIBERI NAMA JELAS
                Route::get('step-3', [TenagaKerjaWizardController::class, 'showStep3'])->name('step3');
                Route::post('step-3', [TenagaKerjaWizardController::class, 'postStep3'])->name('post_step3'); // <-- DIBERI NAMA JELAS
                Route::get('step-4', [TenagaKerjaWizardController::class, 'showStep4'])->name('step4');
                Route::post('step-4', [TenagaKerjaWizardController::class, 'postStep4'])->name('post_step4'); // <-- DIBERI NAMA JELAS
            });
