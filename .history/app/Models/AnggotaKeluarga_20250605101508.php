...
class AnggotaKeluarga extends Model
{
    ...
    public function getFormattedNikAttribute(): string
    {
        ...
        if (empty($nik)) {
            return '-'; // Atau string kosong '' jika lebih disukai
        }

        // Hapus karakter non-numerik (jaga-jaga jika ada spasi atau karakter aneh)
        $cleanedNik = preg_replace('/[^0-9]/', '', (string) $nik);

        if (strlen($cleanedNik) == 16) {
            // Format: 6 angka spasi 6 angka spasi 4 angka
            return substr($cleanedNik, 0, 6) . ' ' . substr($cleanedNik, 6, 6) . ' ' . substr($cleanedNik, 12, 4);
        }

        // Jika setelah dibersihkan panjangnya tidak 16, kembalikan NIK yang sudah dibersihkan (atau NIK asli)
        return $cleanedNik; // Atau return $nik;
    }
    ...
    {
        // Pastikan atribut ada sebelum diakses untuk menghindari error jika kolom null
    ...
    public function getPendidikanTerakhirTextAttribute(): string
    {
        ...
        switch ($this->attributes['pendidikan_terakhir']) {
            ...
            case '6': return 'Tidak Pernah Sekolah'; // Anda menambahkan ini, bagus!
            default: return 'Data Tidak Terverifikasi';
        }
    }
    ...
    {
        // Hanya tampilkan jenis pekerjaan jika statusnya "Bekerja"
        if (isset($this->attributes['status_pekerjaan']) && $this->attributes['status_pekerjaan'] == '1') {
            if (!isset($this->attributes['jenis_pekerjaan']) || $this->attributes['jenis_pekerjaan'] === null || $this->attributes['jenis_pekerjaan'] === '') {
                return 'Jenis Pekerjaan Tidak Diisi'; // Atau '-' jika lebih disukai
            }
        ...
        return '-'; // Jika status pekerjaan bukan '1' (Bekerja)
    }
    ...
    {
        // Hanya tampilkan sub jenis pekerjaan jika statusnya "Bekerja" dan jenis pekerjaan ada
        // Atau bisa juga ditampilkan terlepas dari status pekerjaan jika memang sub_jenis_pekerjaan bisa diisi mandiri
        // Untuk saat ini, saya buat agar bisa tampil meskipun tidak bekerja, sesuai dengan accessor yang Anda berikan
     ...
     * Disesuaikan agar '5' adalah 'Tidak Ada Pendapatan' agar konsisten dengan screenshot.
     * Jika kode '0' yang seharusnya 'Tidak Ada Pendapatan', silakan disesuaikan.
     */
    public function getPendapatanPerBulanTextAttribute(): string
    {
        if (!isset($this->attributes['pendapatan_per_bulan']) || $this->attributes['pendapatan_per_bulan'] === null) {
             // Jika ingin '-' untuk null, bukan "Data Tidak Terverifikasi"
            return '-';
        }
        switch ($this->attributes['pendapatan_per_bulan']) {
            // case '0': return 'Tidak Ada Pendapatan'; // Jika kode '0' yang benar
            ...
            case '5': return 'Tidak Ada Pendapatan'; // Sesuai screenshot, jika kode '5'
            default: return 'Data Tidak Terverifikasi';
        }
    }
}