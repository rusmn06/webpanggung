...
class UserTenagaKerjaController extends Controller
{
    ...
    public function exportExcel($id)
    {
        ...
        try {
            ...
            // PERBAIKAN: `use ($Coordinate)` DIHAPUS
            ...
            // PERBAIKAN: `use ($Coordinate)` DIHAPUS
            $fillNumberPerDigit = function(Worksheet $currentSheet, $numberString, $numDigitsToFill, $startColumn, $row, $padChar = '0') { // Pastikan tidak ada 'use ($Coordinate)' di sini
                $currentColIndex = Coordinate::columnIndexFromString($startColumn); // Menggunakan Class Coordinate
                // Pastikan numberString adalah string dan di-pad dengan karakter yang benar
                $paddedNumberString = str_pad((string)$numberString, $numDigitsToFill, $padChar, STR_PAD_LEFT);

                // Loop sebanyak jumlah digit/kolom yang diinginkan
                for ($i = 0; $i < $numDigitsToFill; $i++) { 
                    // Ambil karakter dari string yang sudah di-pad
                    // Jika $paddedNumberString lebih pendek dari $numDigitsToFill (seharusnya tidak terjadi jika $numberString tidak lebih panjang dari $numDigitsToFill),
                    // maka akan diisi dengan $padChar. Namun, str_pad seharusnya sudah memastikan panjangnya.
                    ...
                    // Opsional: Jika ingin angka rata tengah per sel digit
                    // $currentSheet->getStyle($cellCoordinate)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
                }
            };
            ...
            $fillNumberPerDigit($sheet, $rumahTangga->rt, 3, 'O', 6, '0'); // RT di O6, P6, Q6. Padding dengan '0'
$sheet->setCellValue('R6', '/'); // Pemisah
$fillNumberPerDigit($sheet, $rumahTangga->rw, 3, 'S', 6, '0'); // RW di S6, T6, U6. Padding dengan '0'
            ...
            foreach ($anggotaKeluargaPertama9 as $index => $anggota) {
                ...
                $sheet->setCellValue('AY' . $barisInfoSaatIni, $anggota->pendapatan_per_bulan); // Sesuai klarifikasi Anda
            }
