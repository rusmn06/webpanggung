...
class UserTenagaKerjaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->filled('search')) {
            ...
            $itemsQuery->where(function($q) use ($searchTerm) {
                // Selalu cari di kolom-kolom teks ini
                ...
                // JIKA kata kuncinya adalah ANGKA, cari JUGA di kolom nomor urut
                if (is_numeric($searchTerm)) {
                    $q->orWhere('user_sequence_number', $searchTerm);
                }
            });
        }
    
        // Mengembalikan ke paginate(8) sesuai permintaan sebelumnya
    ...
    {
        $itemsQuery = RumahTangga::where('user_id', Auth::id());
        ...
        $items = $itemsQuery->orderBy('user_sequence_number', 'desc')->paginate(8);

        // Kembalikan data sebagai JSON, bukan view
