...
class TenagaKerjaVerifController extends Controller
{
    ...
    /**
     * Memproses update data dari form edit admin.
     */
    public function update(Request $request, $id)
    {
        // Admin memiliki kuasa lebih, kita bisa gunakan validasi yang sedikit berbeda
        // atau gunakan FormRequest yang sama jika aturannya relevan.
        // Di sini kita gunakan validasi inline untuk contoh.
        $validatedData = $request->validate([
            // Salin semua aturan validasi dari StoreTenagaKerjaRequest ke sini jika diperlukan
        ...
        DB::beginTransaction();
        try {
            // Logika update sama persis seperti di TenagaKerjaController milik user
            
            // 1. Kalkulasi ulang rekapitulasi
            $recap = [
            ...
            $updateData = array_merge(Arr::except($validatedData, [/*...field array...*/]), $recap);
            
            // 3. Handle TTD jika diganti
            if ($request->hasFile('ttd_pendata')) {
                // ... (logika hapus file lama dan simpan yang baru) ...
            }
            
            // 4. Update data RumahTangga
            $item->update($updateData);

            // 5. Hapus dan buat ulang anggota keluarga
            $item->anggotaKeluarga()->delete();
            foreach ($validatedData['nama'] as $index => $nama) {
                // ... (logika create AnggotaKeluarga) ...
            }
