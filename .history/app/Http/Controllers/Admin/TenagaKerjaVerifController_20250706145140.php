...
class TenagaKerjaVerifController extends Controller
{
    ...
    {
        // 1. VALIDASI LENGKAP (GABUNGAN)
        $validatedData = $request->validate([
            'provinsi' => 'required|string|max:100', 'kabupaten' => 'required|string|max:100',
            'kecamatan' => 'required|string|max:100', 'desa' => 'required|string|max:100',
            'rt' => 'required|numeric', 'rw' => 'required|numeric',
            'tgl_pembuatan' => 'required|date', 'nama_pendata' => 'required|string|max:100',
            'nama_responden' => 'required|string|max:100', 'jpr2rtp' => 'required|in:1,2,3,4,5',
            'verif_nama_pendata' => 'required|string|max:100', 'verif_tgl_pembuatan' => 'required|date',
            'ttd_pendata' => 'sometimes|nullable|image|mimes:jpeg,png,jpg|max:2048',
            'nama' => 'required|array|min:1', 'nama.*' => 'required|string|max:100',
            'nik' => 'required|array|min:1', 'nik.*' => 'required|string|digits:16',
            ...
            'jenis_pekerjaan' => 'required|array', 'jenis_pekerjaan.*' => 'required|integer',
            'sub_jenis_pekerjaan' => 'required|array', 'sub_jenis_pekerjaan.*' => 'required|integer',
            'pendapatan_per_bulan' => 'required|array', 'pendapatan_per_bulan.*' => 'required|integer',
            'status' => 'required|in:validated,rejected,pending',
            'admin_tgl_validasi' => 'required_if:status,validated|nullable|date',
            'admin_nama_kepaladusun' => 'required_if:status,validated|nullable|string|max:100',
            'admin_ttd_pendata' => 'sometimes|nullable|image|mimes:jpeg,png,jpg|max:2048',
            'admin_catatan' => 'required_if:status,rejected|nullable|string|max:500',
        ]);
        ...
        DB::beginTransaction();
        try {
            // 2. KALKULASI ULANG REKAPITULASI
            $recap = [
            ...
            // 3. SIAPKAN DATA PENGAJUAN UNTUK DIUPDATE
            $updateDataPengajuan = Arr::except($validatedData, [
                'status', 'admin_tgl_validasi', 'admin_nama_kepaladusun', 'admin_ttd_pendata', 'admin_catatan',
                'nama', 'nik', 'kelamin', 'hdkrt', 'hdkk', 'nuk', 'status_perkawinan', 
                'status_pekerjaan', 'pendidikan_terakhir', 'jenis_pekerjaan', 
                'sub_jenis_pekerjaan', 'pendapatan_per_bulan'
            ]);
            $item->update(array_merge($updateDataPengajuan, $recap));
            
            // 4. UPDATE ANGGOTA KELUARGA
            $item->anggotaKeluarga()->delete();
            foreach ($validatedData['nama'] as $index => $nama) {
                AnggotaKeluarga::create([
                    'rumah_tangga_id' => $item->id, 'nama' => $nama,
                    'nik' => $validatedData['nik'][$index], 'kelamin' => $validatedData['kelamin'][$index],
                    'hdkrt' => $validatedData['hdkrt'][$index], 'hdkk' => $validatedData['hdkk'][$index],
                    'nuk' => $validatedData['nuk'][$index], 'status_perkawinan' => $validatedData['status_perkawinan'][$index],
                    'status_pekerjaan' => $validatedData['status_pekerjaan'][$index], 'pendidikan_terakhir' => $validatedData['pendidikan_terakhir'][$index],
            ...
            // 5. SIAPKAN DAN UPDATE DATA VERIFIKASI
            $updateDataVerifikasi = ['status_validasi' => $validatedData['status']];
            if ($validatedData['status'] === 'validated') {
                if ($request->hasFile('admin_ttd_pendata')) {
                    if ($item->admin_ttd_pendata) Storage::disk('public')->delete('ttd/admin/' . $item->admin_ttd_pendata);
                    $path = $request->file('admin_ttd_pendata')->store('ttd/admin', 'public');
                    $updateDataVerifikasi['admin_ttd_pendata'] = basename($path);
                }
                $updateDataVerifikasi['admin_tgl_validasi'] = $validatedData['admin_tgl_validasi'];
                $updateDataVerifikasi['admin_nama_kepaladusun'] = $validatedData['admin_nama_kepaladusun'];
                $updateDataVerifikasi['admin_catatan'] = $validatedData['admin_catatan'] ?? null;
            } elseif ($validatedData['status'] === 'rejected') {
                $updateDataVerifikasi['admin_catatan'] = $validatedData['admin_catatan'];
            }
            $item->update($updateDataVerifikasi);

            DB::commit();
            return redirect()->route('admin.tkw.show', $item->id)->with('success', 'Data pengajuan dan status verifikasi berhasil diperbarui!');
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Admin gagal update & verifikasi data: ' . $e->getMessage());
            return back()->withInput()->with('error', 'Terjadi kesalahan saat memproses data.');
        }
    }
    ...
    public function destroy($id)
    {
        ...
        // Simpan nomor RT untuk redirect sebelum data dihapus
        $rt = $item->rt;

        DB::beginTransaction();
        try {
            // 2. Hapus file TTD dari storage jika ada, untuk menjaga kebersihan
            if ($item->ttd_pendata) {
            ...
            // 3. Hapus data anggota keluarga terkait
            // (Meskipun cascade di database lebih baik, ini cara eksplisit yang aman)
            $item->anggotaKeluarga()->delete();

            // 4. Hapus data utama Rumah Tangga
            $item->delete();

            DB::commit();

            // 5. Redirect kembali ke halaman daftar RT dengan pesan sukses
            return redirect()->route('admin.tkw.showrt', ['rt' => $rt])
                             ->with('success', 'Data pengajuan berhasil dihapus secara permanen.');

        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Gagal menghapus data pengajuan: ' . $e->getMessage());
    ...
    try {
            ...
            Log::error("PhpSpreadsheet Exception in exportExcel: " . $e->getMessage() . "\n" . $e->getTraceAsString());
            return redirect()->back()->with('error', 'Gagal membuat file Excel (PhpSpreadsheet): ' . $e->getMessage());
        } catch (\Exception $e) {
            Log::error("General Exception in exportExcel: " . $e->getMessage() . "\n" . $e->getTraceAsString());
