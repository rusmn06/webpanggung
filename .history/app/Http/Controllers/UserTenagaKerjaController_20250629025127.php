...
class UserTenagaKerjaController extends Controller
{
    ...
    {
        $userId = Auth::id();
        $baseQuery = RumahTangga::where('user_id', $userId);

        $totalPengajuan = (clone $baseQuery)->count();
        $pengajuanPending = (clone $baseQuery)->where('status_validasi', 'pending')->count();
        $pengajuanDisetujui = (clone $baseQuery)->where('status_validasi', 'validated')->count();
        $pengajuanDitolak = (clone $baseQuery)->where('status_validasi', 'rejected')->count();
        
        $itemsQuery = RumahTangga::where('user_id', $userId);
        
        if ($request->filled('search')) {
        // 1. Ambil kata kunci pencarian lengkap
        $searchTerm = $request->search;
        
        // 2. Buat versi numerik dari kata kunci (hanya angka yang diambil)
        // Contoh: "pengajuan ke 4" akan menjadi "4"
        $numericOnly = preg_replace('/[^0-9]/', '', $searchTerm);

        // 3. Modifikasi query dengan logika yang lebih pintar
        $itemsQuery->where(function($q) use ($searchTerm, $numericOnly) {
            // Pencarian berdasarkan teks di kolom-kolom string
            $q->where('nama_responden', 'like', "%{$searchTerm}%")
              ->orWhere('desa', 'like', "%{$searchTerm}%")
              ->orWhere('kecamatan', 'like', "%{$searchTerm}%")
              ->orWhere('status_validasi', 'like', "%{$searchTerm}%");

            // Jika versi numerik dari pencarian valid (bukan string kosong),
            // cari juga di kolom nomor urut
            if (!empty($numericOnly)) {
                $q->orWhere('user_sequence_number', $numericOnly);
            }
        });
    }
    
        $items = $itemsQuery->orderBy('user_sequence_number', 'desc')
                       ->paginate(6)
                       ->withQueryString(); 
    ...
    // debugging sequence
    // public function fixSequenceNumbers()
    // {
    //     // 1. Ambil semua ID pengguna yang pernah membuat pengajuan
    //     $userIds = RumahTangga::select('user_id')->distinct()->pluck('user_id');

    //     $totalUpdated = 0;

    //     // 2. Loop untuk setiap pengguna
    //     foreach ($userIds as $userId) {
    //         // 3. Ambil semua pengajuan milik pengguna ini, urutkan dari yang paling lama
    //         $submissions = RumahTangga::where('user_id', $userId)
    //             ->orderBy('created_at', 'asc')
    //             ->get();

    //         $sequence = 1;
    //         // 4. Loop untuk setiap pengajuan dan berikan nomor urut
    //         foreach ($submissions as $submission) {
    //             $submission->user_sequence_number = $sequence;
    //             $submission->save(); // Simpan nomor urut ke database
    //             $sequence++;
    //             $totalUpdated++;
    //         }
    //     }
    ...
    //     return "Proses selesai, Sebanyak " . $totalUpdated . " data pengajuan telah berhasil diperbarui dengan nomor urut yang benar.";
    // }
}