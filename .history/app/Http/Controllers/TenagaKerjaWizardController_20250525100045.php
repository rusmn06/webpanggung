...
class TenagaKerjaWizardController extends Controller
{
    ...
    /**
     * Memvalidasi data Langkah 4 dan menyimpan SEMUA data (Step 1-4)
     * ke tabel RumahTangga dan AnggotaKeluarga menggunakan transaksi.
     */
    public function postStep4(Request $request)
    {
        ...
        if ($request->hasFile('ttd_pendata')) {
            ...
            $step4Data['ttd_pendata'] = basename($path);
        }
        ...
        // Pastikan semua session ada sebelum melanjutkan
        ...
        try {
            ...
            $rumahTangga = RumahTangga::create([
                ...
                'ttd_pendata'       => $step4Data['ttd_pendata'] ?? null,
            ...
            for ($i = 0; $i < $memberCount; $i++) {
                AnggotaKeluarga::create([
                    'rumah_tangga_id'     => $rumahTangga->id, // Kunci relasi!
            ...
            // 3. Jika tidak ada error, simpan semua perubahan ke database
            DB::commit();

            // 4. Hapus data session karena sudah disimpan
            session()->forget(['tk.step1', 'tk.step2', 'tk.step3', 'tk.step4']);

            // 5. Arahkan kembali ke awal dengan pesan sukses
            return redirect()->route('tkw.step1')
                             ->with('success', 'Data rumah tangga dan anggotanya berhasil disimpan!');

        } catch (\Exception $e) {
            // 6. Jika terjadi error di tengah jalan, batalkan semua yang sudah disimpan
            DB::rollBack();

            // Catat error ke log agar mudah dilacak
            Log::error('Gagal menyimpan data wizard: ' . $e->getMessage(), [
                'exception' => $e
            ]);

            // 7. Arahkan kembali ke halaman sebelumnya dengan pesan error
            return back()->withInput()
                         ->with('error', 'Terjadi kesalahan fatal saat menyimpan data. Silakan coba lagi atau hubungi admin.');
        }
    }
}