    ...
    public function update(StoreTenagaKerjaRequest $request, $id)
    {
        ...
        try {
            ...
            $recap = [
                ...
                'jart_ab'   => $statusPekerjaanCollection->where('1')->count(),
                'jart_ms'   => $statusPekerjaanCollection->where('3')->count(),
                'jart_tb'   => $statusPekerjaanCollection->whereIn(['2', '4', '5'])->count(),
            ];

            // Melengkapi Arr::except dengan semua field array anggota
            ...
            foreach ($validatedData['nama'] as $index => $nama) {
                // Melengkapi semua field untuk AnggotaKeluarga
