...
class TenagaKerjaWizardController extends Controller
{
     ...
     * SUDAH DIPERBARUI DENGAN MODEL BARU & PAGINATION
     */
    public function showRtData($rt)
    {
        // 1. Validasi nomor RT (Tetap 1-24 sesuai permintaanmu)
        if ($rt < 1 || $rt > 24) {
        ...
        $items = AnggotaKeluarga::whereHas('rumahTangga', function ($query) use ($rt) {
            $query->where('rt', $rt); // Filter berdasarkan RT
        })
        ->with('rumahTangga') // Eager load relasi RumahTangga (untuk status)
        ->orderBy('nama', 'asc') // Urutkan berdasarkan nama anggota
        ->paginate(15); // Terapkan Pagination (menampilkan 15 data per halaman)
