...
class TenagaKerjaWizardController extends Controller
{
     ...
     */
     (!$step1Data || !$step2Data || !$step3Data) {
            // Sesuaikan nama route ke halaman awal wizard
        ...
        try {
            $rumahTangga = RumahTangga::create([
                ...
                'ttd_pendata'       => $ttdPath,
                // 'admin_nama_kepaladusun' TIDAK diisi di sini oleh user
            ...
            // Redirect ke halaman detail pengajuan yang baru dibuat
            // dan kirim flash session untuk memicu modal
            return redirect()->route('user.submission.tkw.show', $rumahTangga->id) // SESUAIKAN NAMA ROUTE INI
                             ->with('show_success_modal', true)
                             ->with('success_message_title', 'Pengajuan Terkirim!')
                             ->with('success_message_body', 'Selamat! Data rumah tangga Anda telah berhasil dikirim dan menunggu verifikasi oleh admin. Pantau statusnya secara berkala. Terima kasih!');

        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Gagal menyimpan data wizard: ' . $e->getMessage(), ['exception' => $e]);
            return back()->withInput()->with('error', 'Terjadi kesalahan fatal saat menyimpan data. Silakan coba lagi.');
        }
    }
