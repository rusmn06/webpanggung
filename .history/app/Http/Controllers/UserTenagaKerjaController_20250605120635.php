...
class UserTenagaKerjaController extends Controller
{
    ...
    // UserTenagaKerjaController.php - method show()
public function show($id)
{
    ...
    $userAllSubmissionIds = RumahTangga::where('user_id', $userId)
                                    ->orderBy('created_at', 'asc') // KUNCI: Urutan dari terlama ke terbaru
                                    ->pluck('id')
                                    ->toArray();
    ...
    if ($itemSequenceIndex !== false) {
        $item->user_sequence_number = $itemSequenceIndex + 1; // Pengajuan ke-1 adalah yang terlama
    } else {
        $item->user_sequence_number = '?';
    }

    // Pastikan view detail menggunakan $item->user_sequence_number
    // seperti yang sudah Anda sertakan:
