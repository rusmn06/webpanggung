        ...
        try {
            ...
            $fillNumberPerDigit = function(Worksheet $currentSheet, $numberString, $numDigitsToFill, $startColumn, $row, $padChar = '0') use ($Coordinate) {
                ...
                for ($i = 0; $i < $numDigitsToFill; $i++) {
                 ...
                 // Anda bisa atur alignment di template Excel langsung, atau di sini jika perlu per sel/range:
                // $rangeToAlign = Coordinate::stringFromColumnIndex($currentColIndex) . $row . ':' . Coordinate::stringFromColumnIndex($currentColIndex + $numDigitsToFill - 1) . $row;
                // $currentSheet->getStyle($rangeToAlign)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER);
            };
            ...
            foreach ($anggotaKeluargaPertama9 as $index => $anggota) {
                ...
                // TUNGGU KLARIFIKASI ANDA UNTUK KOLOM AY:
                // $sheet->setCellValue('AY' . $barisInfoSaatIni, $anggota->NAMA_FIELD_PENDAPATAN_UNTUK_AY); 
            }
