...
class UserTenagaKerjaController extends Controller
{
    ...
    public function exportExcel($id)
    {
        ...
        try {
            ...
            $fillNumberPerDigit = function(Worksheet $currentSheet, $numberString, $numDigitsToFill, $startColumn, $row, $padChar = '0') use ($Coordinate) {
                $currentColIndex = Coordinate::columnIndexFromString($startColumn);
                ...
                for ($i = 0; $i < $numDigitsToFill; $i++) { 
                    ...
                    // $currentSheet->getStyle($cellCoordinate)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_CENTER); // Center align
                }
            };

            // 1. PENGENALAN TEMPAT (CAPSLOCK)
            ...
            // Nama Pendata di AP4 sampai AZ4 (baris 4)
            $sheet->setCellValue('AP4', $rumahTangga->nama_pendata);
            // Nama Responden di AP6 sampai AZ6 (baris 6)
            ...
            foreach ($anggotaKeluargaPertama9 as $index => $anggota) {
                ...
                $sheet->setCellValue('D' . $barisNamaSaatIni, $anggota->nama); // Nama Anggota (merge D-T)
                $nikString = (string)$anggota->nik;
                if (strlen($nikString) == 16) {
                    $fillNumberPerDigit($sheet, substr($nikString, 0, 6), 6, 'D', $barisInfoSaatIni); // NIK Bagian 1 (D-I)
                    $fillNumberPerDigit($sheet, substr($nikString, 6, 6), 6, 'K', $barisInfoSaatIni); // NIK Bagian 2 (K-P)
                    $fillNumberPerDigit($sheet, substr($nikString, 12, 4), 4, 'R', $barisInfoSaatIni);// NIK Bagian 3 (R-U)
                } else { // Handle NIK tidak 16 digit
                ...
                // Sesuai spesifikasi baru:
                $sheet->setCellValue('X'  . $barisInfoSaatIni, $anggota->hdkrt);                // Hubungan Kepala RT (kode)
                $sheet->setCellValue('AA' . $barisInfoSaatIni, $anggota->nuk);                  // No Urut Anggota Keluarga (kode)
                // $sheet->setCellValue('AD' . $barisInfoSaatIni, $anggota->hdkk);              // Hubungan dengan Kepala Keluarga - Anda punya hdkrt dan hdkk, pastikan mana yang dipakai
                $sheet->setCellValue('AD' . $barisInfoSaatIni, $anggota->kelamin);              // Jenis Kelamin (kode)
                $sheet->setCellValue('AG' . $barisInfoSaatIni, $anggota->status_perkawinan);    // Status Perkawinan (kode)
                $sheet->setCellValue('AJ' . $barisInfoSaatIni, $anggota->status_pekerjaan);     // Status Pekerjaan (kode)
                $sheet->setCellValue('AM' . $barisInfoSaatIni, $anggota->jenis_pekerjaan);     // Jenis Pekerjaan (kode)
                $sheet->setCellValue('AP' . $barisInfoSaatIni, $anggota->sub_jenis_pekerjaan);  // Sub Jenis Pekerjaan (kode)
                $sheet->setCellValue('AS' . $barisInfoSaatIni, $anggota->pendidikan_terakhir);  // Pendidikan Terakhir (kode)
                $sheet->setCellValue('AV' . $barisInfoSaatIni, $anggota->pendapatan_per_bulan); // Pendapatan (kode)
                $sheet->setCellValue('AY' . $barisInfoSaatIni, $anggota->pendapatan_per_bulan); // PENDAPATAN LAGI DI AY (kode) - Sesuai permintaan
            }
            ...
            foreach ($anggotaKeluargaCadangan as $anggota) {
                ...
                $sheet->setCellValue('AA'. $currentRowCadangan, $anggota->sub_jenis_pekerjaan); // Kode
                $sheet->setCellValue('AB'. $currentRowCadangan, $anggota->pendidikan_terakhir_text);
                $sheet->setCellValue('AC'. $currentRowCadangan, $anggota->pendapatan_per_bulan); // Kode
                $counterCadangan++;
            }
            ...
            if ($rumahTangga->verif_tgl_pembuatan) {
                ...
                $fillNumberPerDigit($sheet, $tglVerifPendata->format('d'), 2, 'T', 52); // Tanggal Pendata T52-U52
