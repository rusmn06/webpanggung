...
class UserTenagaKerjaController extends Controller
{
    ...
    {
        // 1. Cari data pengajuan yang sudah ada berdasarkan ID
        $item = RumahTangga::findOrFail($id);

        // 2. Redirect ke halaman detail, sambil membawa "flash data" untuk menampilkan modal
        return redirect()->route('tenagakerja.show', $item->id)
            ->with('show_success_modal', true)
            ->with('success_message_title', '🧪 Uji Coba Pop-up')
            ->with(
                'success_message_body',
                // Kita gunakan user_sequence_number dari data yang ada untuk tes
                'Ini adalah tes. Data Pengajuan Ke-'.$item->user_sequence_number.' akan ditampilkan di sini. Apakah tombol close berfungsi?'
            );
    }
