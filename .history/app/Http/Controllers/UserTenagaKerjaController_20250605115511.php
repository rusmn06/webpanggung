...
{
    $userId = Auth::id();
    $item = RumahTangga::with('anggotaKeluarga') // Model Anda adalah RumahTangga
                        ->where('user_id', $userId)
                        ->findOrFail($id);

    // Mengambil semua ID pengajuan (RumahTangga) milik user, diurutkan dari yang terlama
    $userAllSubmissionIds = RumahTangga::where('user_id', $userId)
                                    ->orderBy('created_at', 'asc') // Penting: 'asc' untuk urutan 1, 2, 3, ...
                                    ->pluck('id')
                                    ->toArray();

    // Mencari posisi item saat ini dalam daftar tersebut
    $itemSequenceIndex = array_search($item->id, $userAllSubmissionIds);

    // Menambahkan nomor urut ke objek $item sebagai properti dinamis
    if ($itemSequenceIndex !== false) {
        // Nomor urut adalah index + 1
        $item->user_sequence_number = $itemSequenceIndex + 1;
    } else {
        // Fallback jika tidak ditemukan (seharusnya tidak terjadi jika data konsisten)
        $item->user_sequence_number = '?'; // Atau '??' seperti sebelumnya
    }

    // Mengirimkan $item (yang kini memiliki properti user_sequence_number) ke view
