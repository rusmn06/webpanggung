...
public function index()
{
    ...
    $items = RumahTangga::where('user_id', $userId) // Buat query baru agar tidak terpengaruh clone
                        ...
                        ->paginate(6); // Atau berapa pun item per halaman Anda

    // Dapatkan semua ID pengajuan user, diurutkan dari terlama (untuk nomor urut absolut)
    $userAllSubmissionIdsOrderedAsc = RumahTangga::where('user_id', $userId)
                                        ->orderBy('created_at', 'asc')
                                        ->pluck('id')
                                        ->toArray();

    // Tambahkan 'user_sequence_number' ke setiap item di halaman saat ini
    ...
    return view('pages.tenagakerja.index', compact(
        'items', // items kini memiliki 'user_sequence_number'
        'totalPengajuan', // Ini adalah total count, bukan nomor urut tertinggi. Bisa sama jika tidak ada data yg dihapus.
        'pengajuanPending',
        'pengajuanDisetujui',
        'pengajuanDitolak'
    ));
