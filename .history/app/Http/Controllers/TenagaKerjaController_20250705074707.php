...
class TenagaKerjaController extends Controller
{
    ...
    public function update(StoreTenagaKerjaRequest $request, $id)
    {
        ...
        // File: app/Http/Controllers/TenagaKerjaController.php -> di dalam metode update()

        DB::beginTransaction();
        try {
            // 1. Kalkulasi ulang rekapitulasi dari data yang divalidasi
            $recap = [
            ...
            // 2. Siapkan data untuk di-update ke tabel utama
            ...
            // 3. Handle TTD jika admin menggantinya
            ...
            // 4. Update data utama RumahTangga
            $item->update($updateData);

            // 5. Hapus anggota keluarga lama dan buat ulang dari data form
            ...
            // 6. Jika status sebelumnya 'ditolak', ubah menjadi 'pending'
            if ($item->status_validasi === 'rejected') {
                $item->status_validasi = 'pending';
                $item->admin_catatan_validasi = null; // Hapus catatan penolakan lama
                $item->save();
            }
