...
<form action="{{ route('tenagakerja.update', $item->id) }}" method="POST" enctype="multipart/form-data" id="main-form">
    ...
    <div class="w-100 d-flex justify-content-end my-4">
        <a href="{{ route('tenagakerja.show', $item->id) }}" class="btn btn-secondary mr-2">Batal & Kembali ke Detail</a>
        <a href="{{ route('tenagakerja.index') }}" class="btn btn-outline-secondary mr-3">Kembali ke Riwayat</a>
        <button type="submit" class="btn btn-primary" @if($item->status_validasi === 'validated') disabled @endif>
