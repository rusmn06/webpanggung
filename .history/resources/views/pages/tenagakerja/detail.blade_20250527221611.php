...
<div class="container-fluid">
    ...
    <div class="col-md-8 mb-4">
        <div class="card shadow-sm h-100"> {{-- h-100 agar tinggi sama (opsional) --}}
            <div class="card-body p-3">
                <div class="row">
                    {{-- Kolom Kiri Dalam Kartu Kiri --}}
                    <div class="col-sm-6">
                        <h6 class="text-dark">Nama Pendata:</h6>
                        <p class="font-weight-normal mb-3">{{ $item->nama_pendata }}</p>

                        <h6 class="text-dark">Nama Responden:</h6>
                        <p class="font-weight-normal mb-3">{{ $item->nama_responden }}</p>
                    </div>

                    {{-- Kolom Kanan Dalam Kartu Kiri --}}
                    <div class="col-sm-6">
                        <h6 class="text-dark mb-2">Lokasi:</h6>
                        <p class="mb-1"><small><strong>Prov:</strong> {{ $item->provinsi ?? 'N/A' }}</small></p>
                        <p class="mb-1"><small><strong>Kab:</strong> {{ $item->kabupaten }}</small></p>
                        <p class="mb-1"><small><strong>Kec:</strong> {{ $item->kecamatan }}</small></p>
                        <p class="mb-1"><small><strong>Desa:</strong> {{ $item->desa }}</small></p>
                        <p class="mb-1"><small><strong>RT/RW:</strong> {{ $item->rt }}/{{ $item->rw }}</small></p>
                    </div>
                </div>
                <hr> {{-- Garis pemisah --}}
                <p class="card-text mb-0 mt-2"><small class="text-muted">
    ...
    {{-- KARTU KANAN (Status & Aksi) --}}
    <div class="col-md-4 mb-4">
        <div class="ca{{--  --}} sama (opsional) --}}
            <div class="card-body p-3 d-flex flex-column justify-content-center align-items-center text-center">
                ...
                <div class="mb-4">
                    ...
                    @endphp
                    <span class="badge {{ $badgeClass }} p-2" style="font-size: 1rem;">Status: {{ $statusText }}</span> {{-- Ukuran font disesuaikan --}}
                </div>

                {{-- Tombol Export --}}
                <div>
                    {{-- Ganti '#' dengan URL/Route untuk export --}}
                    <a href="#" class="btn btn-success">
                        <i class="fas fa-download mr-1"></i> {{-- Icon Download (Font Awesome) --}}
                        Export Excel
                    </a>
                </div>
