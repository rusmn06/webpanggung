...
@push('styles')
    <style>
        ...
        /* Style untuk search input di card header */
        #searchRiwayatInputContainer {
            max-width: 320px; /* Lebar maksimal search input */
        }
    </style>
@endpush
...
@section('content')
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex flex-column flex-sm-row align-items-sm-center justify-content-sm-between">
            <h6 class="m-0 font-weight-bold text-primary mb-2 mb-sm-0">Riwayat Pengajuan Kuesioner Anda</h6>
            @if(!$items->isEmpty())
            <div id="searchRiwayatInputContainer" class="w-100 w-sm-auto"> {{-- Lebar penuh di mobile, auto di sm ke atas --}}
                <input type="text" id="searchRiwayatInput" class="form-control form-control-sm" placeholder="Cari (ID, Responden, Lokasi, Status)...">
            </div>
            @endif
        </div>
        <div class="card-body">
            ...
            @else
                <div class="row" id="riwayatPengajuanContainer">
                    @foreach($items as $item)
                        {{-- Atribut data-date tidak lagi diperlukan jika filter tanggal dihapus --}}
                        <div class="col-xl-4 col-lg-6 mb-4 submission-card-wrapper">
                            <div class="card submission-card shadow-sm h-100">
                                ...
                                <div class="card-footer bg-light text-right py-2 px-3">
                                    <a href="{{ route('tenagakerja.show', $item->id) }}" {{-- Pastikan nama route ini benar --}}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function filterCards() {
        ...
        for (let i = 0; i < cards.length; i++) {
            ...
            // data-date tidak lagi dipakai karena filter tanggal dihapus
            // const cardDataDate = cardWrapper.dataset.date;
            ...
            // Filter tanggal dihapus, jadi dateMatch selalu true
            // let dateMatch = true;

            if (textMatch) { // Hanya periksa textMatch
        ...
        if (noResultsMessage) {
            // Pesan disesuaikan karena hanya ada filter teks
    ...
    // Event listener dan elemen untuk filter tanggal dihapus
    // const startDateFilter = document.getElementById('startDateFilter');
    // const endDateFilter = document.getElementById('endDateFilter');
    // const resetFilterBtn = document.getElementById('resetFilterBtn');
    // if (startDateFilter) { startDateFilter.addEventListener('change', filterCards); }
    // if (endDateFilter) { endDateFilter.addEventListener('change', filterCards); }
