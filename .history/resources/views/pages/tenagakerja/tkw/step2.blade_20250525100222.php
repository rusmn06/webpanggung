...
@section('title', 'Identitas Anggota Keluarga')

@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Identitas Anggota Keluarga (Langkah 2)</h1>
    </div>
    ...
    <div class="card shadow mb-4">
        <div class="card-body">
            <form action="{{ route('tkw.step2') }}" method="POST">
                ...
                {{-- Container for dynamic member forms --}}
                <div id="members-container">

                    {{-- Check if there's old input (validation failed) or existing session data --}}
                    @php
                        // Determine the source of data: old input first, then session, then default to one empty
                        $members = old('nama') ? old('nama') : (isset($data['nama']) && is_array($data['nama']) ? $data['nama'] : ['']);
                        // Ensure we always have at least one form
                    ...
                    @foreach ($members as $index => $nama_value)
                        <div class="member-form card mb-3">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-row">
                                    <div class="form-group col-md-3"> {{-- Adjusted size --}}
                                        <label for="hdkrt_{{ $index }}">Hub. KRT</label>
                                    ...
                                    <div class="form-group col-md-3"> {{-- Adjusted size --}}
                                        <label for="hdkk_{{ $index }}">Hub. KK</label>
                                    ...
                                    <div class="form-group col-md-2"> {{-- Adjusted size & label --}}
                                        <label for="nuk_{{ $index }}">No. Urut Anggota</label>
                                        <input type="number" name="nuk[]" id="nuk_{{ $index }}" min="1" max="99"
                                            value="{{ old('nuk.'.$index, $data['nuk'][$index] ?? ($index + 1)) }}" {{-- Auto-increment or old --}}
                                            class="form-control @error('nuk.'.$index) is-invalid @enderror">
                                        @error('nuk.'.$index)
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div class="form-group col-md-4"> {{-- Adjusted size --}}
                                    ...
                                    <div class="form-group col-md-3"> {{-- Adjusted size --}}
                                        ...
                                        <select name="pendidikan_terakhir[]" class="form-control @error('pendidikan_terakhir.'.$index) is-invalid @enderror" id="pendidikan_terakhir_{{ $index }}">
                                           <option value="">-- Pilih --</option>
                                            ...
                                            <option value="5" {{ old('pendidikan_terakhir.'.$index, $data['pendidikan_terakhir'][$index] ?? '') == 5 ? 'selected' : '' }}>Tamat Perguruan Tinggi</option>
                                    ...
                                    <div class="form-group col-md-3">
                                        ...
                                        <select name="pendapatan_per_bulan[]" class="form-control @error('pendapatan_per_bulan.'.$index) is-invalid @enderror" id="pendapatan_per_bulan_{{ $index }}">
                                            ...
                                            <option value="5" {{ old('pendapatan_per_bulan.'.$index, $data['pendapatan_per_bulan'][$index] ?? '') == 5 ? 'selected' : '' }}>Tidak Berpenghasilan</option>
                                             {{-- Asumsi ada value 6 berdasarkan controller --}}
                                            <option value="6" {{ old('pendapatan_per_bulan.'.$index, $data['pendapatan_per_bulan'][$index] ?? '') == 6 ? 'selected' : '' }}>[Opsi 6 - Sesuaikan]</option>
                                        </select>
                ...
                <div class="form-row">
                    <div class="form-group col-md-3">
                        ...
                        <select name="pendapatan_per_bulan[]" class="form-control">
                            ...
                            <option value="6">[Opsi 6 - Sesuaikan]</option>
                        </select>
                    </div>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const template = document.getElementById('member-template').firstElementChild.cloneNode(true); // Get the 'member-form' div
        ...
        function updateMemberNumbers() {
            ...
            forms.forEach((form, index) => {
                ...
                // Update labels and ids (optional but good practice)
