...
document.addEventListener('DOMContentLoaded', function() {
    function liveValidateInput(event) {
        ...
        // ... (logika validasi string-only dan numeric-only yang sudah ada) ...

        // TAMBAHKAN BLOK KODE INI
        // Cek jika input adalah type="number" dan punya atribut maxlength
        if (input.type === 'number' && input.hasAttribute('maxlength')) {
            // Ambil nilai maxlength
            const maxLength = parseInt(input.getAttribute('maxlength'), 10);
            // Jika panjang value saat ini melebihi maxlength
            if (input.value.length > maxLength) {
                // Potong value agar sesuai dengan maxlength
