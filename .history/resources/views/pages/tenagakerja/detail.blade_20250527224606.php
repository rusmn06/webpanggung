...
<div class="container-fluid">
    ...
    {{-- KARTU KIRI (col-8) - Detail Informasi --}}
    <div class="col-8 mb-4">
        <div class="card shadow-sm card-detail-section h-100"> {{-- h-100 agar tinggi sama --}}
            <div class="card-body p-3">
                ...
                {{-- Bagian Lokasi --}}
                <div class="mt-3">
                    <h6 class="text-dark mb-2">Alamat Lengkap:</h6>
                    {{-- (Jika ingin titik dua rata, gunakan tabel dari jawaban sebelumnya) --}}
                    <p class="mb-1"><strong>Provinsi:</strong> {{ $item->provinsi }}</p>
                    <p class="mb-1"><strong>Kota/Kab:</strong> {{ $item->kabupaten }}</p>
                    <p class="mb-1"><strong>Kecamatan:</strong> {{ $item->kecamatan }}</p>
                    <p class="mb-1"><strong>Desa/Kelurahan:</strong> {{ $item->desa }}</p>
                    <p class="mb-0"><strong>RT/RW:</strong> {{ $item->rt }} / {{ $item->rw }}</p>
                </div>
    ...
    {{-- KARTU KANAN (col-4) - Status & Aksi --}}
    <div class="col-4 mb-4">
        <div class="card shadow-sm h-100"> {{-- h-100 agar tinggi sama --}}
            <div class="card-body p-3 d-flex flex-column"> {{-- d-flex & flex-column untuk layout vertikal --}}

                <h5 class="card-title text-center text-dark mb-3">Status & Aksi</h5>
                <hr class="mt-0">

                {{-- Status --}}
                <div class="text-center mb-3">
                    <h6 class="text-muted mb-2">Status Validasi</h6>
                    @php
                    ...
                    @endphp
                    <span class="badge {{ $badgeClass }} p-2" style="font-size: 1rem; width: 100%;">{{ $statusText }}</span>
                </div>

                {{-- Tanggal Pengajuan --}}
                <div class="text-center mb-4">
                     <small class="text-muted">
                        Diajukan pada: {{ \Carbon\Carbon::parse($item->tgl_pembuatan)->isoFormat('dddd, D MMMM YYYY') }}
                    </small>
                </div>

                {{-- Tombol Aksi --}}
                <div class="mt-auto text-center"> {{-- mt-auto: Mendorong ke bawah --}}
                    <h6 class="text-muted mb-2">Ekspor Data</h6>
                    <a href="#" class="btn btn-success w-100 mb-2"> {{-- Ganti # dengan URL --}}
