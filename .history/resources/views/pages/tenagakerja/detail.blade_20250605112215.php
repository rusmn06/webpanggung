...
<style>
    ...
    /* PERUBAHAN: Penyesuaian gaya Kartu Export */
    .card-export-final .card-body {
        padding: 1.5rem; /* Sedikit menambah padding agar tidak terlalu mepet */
        display: flex;
        align-items: center; /* Konten di tengah secara vertikal */
    }

    .card-export-final .icon-export-display { /* Ubah nama kelas agar lebih generik */
        font-size: 4.5rem;  /* Ukuran ikon sedikit disesuaikan */
        color: #1D6F42; /* Warna hijau khas Excel, atau #28a745 jika ingin sama dengan tombol */
        margin-right: 1.5rem; /* Jarak antara ikon dan teks */
        line-height: 1; /* Menjaga ikon tetap rapi */
    }

    .card-export-final .export-content-wrapper {
        display: flex;
        flex-direction: column;
        justify-content: center; /* Teks dan tombol di tengah secara vertikal */
    }

    .card-export-final .export-title {
        font-size: 1.1rem; /* Judul kecil untuk bagian ekspor */
        font-weight: 600;
        color: #333;
        margin-bottom: 0.35rem;
    }

    .card-export-final .export-text-description {
        font-size: 0.875rem;
        color: #555; /* Warna teks sedikit lebih gelap untuk kontras */
        line-height: 1.5;
        margin-bottom: 1rem; /* Jarak antara deskripsi dan tombol */
    }

    .card-export-final .btn-download-action {
        font-size: 0.9rem;
        align-self: flex-start; /* Tombol rata kiri dalam wrapper-nya */
    }

    /* Untuk layar kecil (mobile), ikon bisa dikecilkan atau disembunyikan agar tidak memakan tempat */
    @media (max-width: 767.98px) {
        .card-export-final .card-body {
            padding: 1rem;
            flex-direction: column; /* Tata letak menjadi kolom di mobile */
            text-align: center; /* Teks di tengah */
        }
        .card-export-final .icon-export-display {
            font-size: 3.5rem;
            margin-right: 0; /* Hapus margin kanan */
            margin-bottom: 0.75rem; /* Beri jarak bawah ke teks */
        }
        .card-export-final .export-content-wrapper {
...
<div class="container-fluid">
    ...
    <div class="row d-flex align-items-stretch">
        ...
        <div class="col-lg-5 d-flex flex-column">
            ...
            {{-- PERUBAHAN: Kartu Export dengan Tata Letak dan Konten yang Diperbarui --}}
    <div class="card shadow-sm card-export-final flex-grow-1">
        <div class="card-body"> {{-- Flex sudah diatur di CSS --}}
            {{-- Kolom Ikon (Kiri) --}}
            {{-- Menggunakan ikon file-excel dari Font Awesome --}}
            <div class="d-none d-md-flex align-items-center"> {{-- Sembunyikan ikon di layar sangat kecil jika perlu, atau sesuaikan ukurannya via @media --}}
                <i class="fas fa-file-excel icon-export-display"></i>
            </div>

            {{-- Kolom Konten Teks & Tombol (Kanan) --}}
            <div class="export-content-wrapper">
                <h6 class="export-title">Ekspor Data Pengajuan</h6>
                <p class="export-text-description">
                    Unduh data pengajuan lengkap Anda dalam format Excel (XLSX). File ini mencakup semua detail yang telah diinput, termasuk data anggota keluarga, dan dapat digunakan untuk arsip atau analisis lebih lanjut.
                </p>
                <a href="{{ route('tenagakerja.exportExcel', ['id' => $item->id]) }}" class="btn btn-success btn-download-action">
