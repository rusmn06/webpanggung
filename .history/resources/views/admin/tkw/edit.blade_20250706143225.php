...
<style>
    ...
    .signature-box { border: 2px dashed #d1d3e2; border-radius: .35rem; padding: 1.5rem; text-align: center; display: flex; flex-direction: column; justify-content: center; background-color: #f8f9fc; height: 80%; }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const userSignaturePreview = document.getElementById('signature-preview-user');
    const userFileInfo = document.getElementById('user-file-info');
    const ttdAdminInput = document.getElementById('admin_ttd_pendata');
    const adminSignaturePreview = document.getElementById('signature-preview-admin');
    ...
    function setupSignaturePreview(inputEl, previewEl, infoEl = null) {
        if (inputEl && previewEl) {
            inputEl.addEventListener('change', function (event) {
                ...
                if (infoEl) {
                    ...
                    infoEl.innerHTML = `<span class="text-muted">${file.name} (${fileSize})</span>`;
                }
            });
        }
    }
    ...
    function updateVerificationFormState() {
        ...
        if (commentTagsContainer) commentTagsContainer.style.display = 'none';
        ...
        if (radioValidated.checked) {
            ...
            if (commentTagsContainer) commentTagsContainer.style.display = 'block';
        }
    }
    ...
    setupSignaturePreview(ttdUserInput, userSignaturePreview, userFileInfo);
    setupSignaturePreview(ttdAdminInput, adminSignaturePreview);
    ...
    if (commentTags) {
        commentTags.forEach(tag => {
            tag.addEventListener('click', function () {
                ...
                commentTextarea.value = currentText ? currentText + ' ' + tagText : tagText;
            });
        });
    }
