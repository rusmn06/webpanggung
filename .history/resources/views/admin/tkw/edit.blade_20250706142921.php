            ...
            <div class="col-lg-5 mb-4">
                <div class="card shadow-sm h-100 d-flex flex-column">
                    ...
                    <div class="card-body d-flex flex-column">
                        {{-- Tag <form> yang bersarang sudah DIHAPUS dari sini --}}
                        ...
                        <div class="mb-3">
                            ...
                            <div id="signature-preview-admin" class="mt-2"> 
                                @if($item->admin_ttd_pendata)
                                    <img src="{{ asset('storage/ttd/admin/'.$item->admin_ttd_pendata) }}" style="max-height: 80px; border: 1px solid #ddd;">
                                @else
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-4">
                                ...
                                <select name="jenis_pekerjaan[]" class="form-control @error('jenis_pekerjaan.'.$index) is-invalid @enderror">
                                    <option value="">Pilih...</option>
                                    @foreach(['1'=>'PNS/TNI/POLRI','2'=>'Karyawan/Honorer','3'=>'Wiraswasta','4'=>'Lainnya'] as $key => $value)
                                        <option value="{{$key}}" @if(old('jenis_pekerjaan.'.$index, optional($anggota)->jenis_pekerjaan) == $key) selected @endif>{{$value}}</option>
                                    @endforeach
                                </select>
                                @error('jenis_pekerjaan.'.$index)<div class="invalid-feedback">{{$message}}</div>@enderror
                            </div>
                            <div class="form-group col-md-4">
                                <label>Sub-Jenis Pekerjaan</label>
                                <select name="sub_jenis_pekerjaan[]" class="form-control @error('sub_jenis_pekerjaan.'.$index) is-invalid @enderror">
                                    <option value="">Pilih...</option>
                                    @foreach(['1'=>'Aparatur','2'=>'Profesional','3'=>'Tenaga Harian','4'=>'Wira Usaha','5'=>'Lainnya'] as $key => $value)
                                        <option value="{{$key}}" @if(old('sub_jenis_pekerjaan.'.$index, optional($anggota)->sub_jenis_pekerjaan) == $key) selected @endif>{{$value}}</option>
                                    @endforeach
                                </select>
                                @error('sub_jenis_pekerjaan.'.$index)<div class="invalid-feedback">{{$message}}</div>@enderror
                            </div>
                            <div class="form-group col-md-4">
                                <label>Pendapatan per Bulan</label>
                                <select name="pendapatan_per_bulan[]" class="form-control @error('pendapatan_per_bulan.'.$index) is-invalid @enderror">
                                    <option value="">Pilih...</option>
                                    @foreach(['1'=>'> 500 ribu','2'=>'> 1 juta','3'=>'> 2 juta','4'=>'> 4 juta','5'=>'Tdk Berpenghasilan'] as $key => $value)
                                        <option value="{{$key}}" @if(old('pendapatan_per_bulan.'.$index, optional($anggota)->pendapatan_per_bulan) == $key) selected @endif>{{$value}}</option>
                                    @endforeach
                                </select>
                                @error('pendapatan_per_bulan.'.$index)<div class="invalid-feedback">{{$message}}</div>@enderror
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="card-body">
                <div class="row align-items-stretch">
                    ...
                    <div class="col-md-6">
                        ...
                        {{-- Input file yang tersembunyi tetap di atas --}}
                        <input type="file" name="ttd_pendata" id="ttd_pendata" class="d-none @error('ttd_pendata') is-invalid @enderror" accept="image/png, image/jpeg">
                        
                        {{-- Kontainer baru untuk preview --}}
                        <div class="signature-box" id="user-signature-container">
                            {{-- Area ini akan diisi oleh JavaScript --}}
                            <div id="signature-preview-user">
                                @if($item->ttd_pendata)
                                    <img src="{{ asset('storage/ttd/pendata/'.$item->ttd_pendata) }}" alt="Tanda Tangan Saat Ini" style="max-height: 120px; max-width: 100%; border-radius: .25rem; margin-bottom: 1rem;">
                                @else
                                    <i class="fas fa-signature fa-3x text-gray-400 mb-3"></i>
                                    <p class="text-muted small">Belum ada tanda tangan.</p>
                                @endif
                            </div>

                            {{-- Info file akan muncul di sini --}}
                            <div id="user-file-info" class="mt-2" style="font-size: 0.8rem;"></div>
                        </div>
                        
                        {{-- Tombol untuk memilih file dan pesan error --}}
                        <div class="mt-2 text-center">
                            <label for="ttd_pendata" class="btn btn-sm btn-outline-primary">
                                <i class="fas fa-upload mr-2"></i> Ganti Tanda Tangan (Opsional)
                            </label>
                            @error('ttd_pendata')<div class="invalid-feedback d-block mt-1">{{ $message }}</div>@enderror
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="member-form card mb-3">
        ...
        <div class="card-body">
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-3"><label>Hub. KK</label><select name="hdkk[]" class="form-control"><option value="">Pilih...</option><option value="1">Kepala Keluarga</option><option value="2">Istri / Suami</option><option value="3">Anak</option><option value="4">Menantu</option><option value="5">Cucu</option><option value="6">Orang Tua / Mertua</option><option value="7">Pembantu</option><option value="8">Lainnya</option></select></div>
                {{-- Ini adalah versi yang sudah diperbaiki tanpa variabel PHP --}}
                <div class="form-group col-md-3"><label>No. Urut</label><input type="number" name="nuk[]" min="1" max="99" class="form-control" readonly></div>
            </div><hr>
            <div class="form-row">
            ...
            <div class="form-row">
                <div class="form-group col-md-4">
                    ...
                    <select name="jenis_pekerjaan[]" class="form-control">
                        <option value="">Pilih...</option><option value="1">PNS/TNI/POLRI</option><option value="2">Karyawan/Honorer</option><option value="3">Wiraswasta</option><option value="4">Lainnya</option>
                    </select>
                ...
                <div class="form-group col-md-4">
                    ...
                    <select name="sub_jenis_pekerjaan[]" class="form-control">
                        <option value="">Pilih...</option><option value="1">Aparatur</option><option value="2">Profesional</option><option value="3">Tenaga Harian</option><option value="4">Wira Usaha</option><option value="5">Lainnya</option>
                    </select>
                    <div class="invalid-feedback">Field ini wajib diisi.</div>
                </div>
                <div class="form-group col-md-4">
                    ...
                    <select name="pendapatan_per_bulan[]" class="form-control">
                        <option value="">Pilih...</option><option value="1">> 500 ribu</option><option value="2">> 1 juta</option><option value="3">> 2 juta</option><option value="4">> 4 juta</option><option value="5">Tdk Berpenghasilan</option>
                    </select>
                    <div class="invalid-feedback">Field ini wajib diisi.</div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    // ======================================================
    // BAGIAN 1: DEKLARASI ELEMEN & DATA AWAL
    // ======================================================
    const initialVerificationData = {
        ...
        catatan: `{!! str_replace(["\r", "\n"], ' ', e($item->admin_catatan)) !!}` // e() untuk XSS protection
    };

    const mainForm = document.getElementById('main-form');
    if (!mainForm) return;

    // Anggota Keluarga & Rekap
    ...
    const signatureUserPreview = document.getElementById('signature-preview-user');
    const ttdAdminInput = document.getElementById('admin_ttd_pendata');
    const signatureAdminPreview = document.getElementById('signature-preview-admin'); // Deklarasi ganda dihapus

    // Verifikasi Admin
    const radioValidated = document.getElementById('status_validated');
    const radioRejected = document.getElementById('status_rejected');
    // const radioPending = document.getElementById('status_pending'); // Dihapus karena tidak ada di HTML
    ...
    /**
     * [FUNGSI BARU] Fungsi untuk preview gambar TTD.
     * Dibuat agar bisa dipakai ulang untuk TTD user dan admin.
     */
    function setupSignaturePreview(inputEl, previewEl) {
        if (inputEl && previewEl) {
            inputEl.addEventListener('change', function(event) {
                previewEl.innerHTML = ''; // Kosongkan preview lama
    ...
    /**
     * [FUNGSI DIPERBAIKI] Fungsi yang hilang untuk menomori ulang anggota.
     */
    function updateAllMemberNumbers() {
        ...
        forms.forEach((form, index) => {
            ...
            if(header) header.textContent = `Anggota Keluarga #${index + 1}`;
    ...
    /**
     * [FUNGSI DIPERBAIKI] Ditambahkan pemanggilan updateAllMemberNumbers.
     */
    function addMemberForm() {
        ...
        updateAllMemberNumbers(); // <-- Panggilan yang hilang ditambahkan
        updateRecap();
    }
    ...
    if (container) {
        container.addEventListener('click', e => {
            if (e.target.closest('.remove-member-btn')) {
                e.preventDefault();
                if (container.querySelectorAll('.member-form').length > 1) {
    ...
    // [BARU] Setup preview untuk kedua input TTD
