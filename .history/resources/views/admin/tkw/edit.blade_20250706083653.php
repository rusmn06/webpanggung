                        ...
                        <form id="verification-form" action="{{ route('admin.tkw.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="date" name="admin_tgl_validasi" id="admin_tgl_validasi"
                                    value="{{ old('admin_tgl_validasi', optional($item->admin_tgl_validasi)->format('Y-m-d') ?? now()->toDateString()) }}"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateFormState() {
        ...
        if (radioValidated.checked) {
            ...
            // Kembalikan nilai lama dari DB jika user toggle kembali ke validated
            tglValidasiInput.value = tglValidasiInput.dataset.oldValue || tglValidasiInput.value;
            namaVerifikatorInput.value = namaVerifikatorInput.dataset.oldValue || namaVerifikatorInput.value;
            if (!signaturePreview.innerHTML.trim() && signaturePreview.dataset.oldSignature) {
                signaturePreview.innerHTML = signaturePreview.dataset.oldSignature;
            }
            ...
            tglValidasiInput.dataset.oldValue = tglValidasiInput.value;
            namaVerifikatorInput.dataset.oldValue = namaVerifikatorInput.value;
            signaturePreview.dataset.oldSignature = signaturePreview.innerHTML;
