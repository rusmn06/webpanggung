...
@section('content')
    ...
    {{-- Judul Halaman dan Tombol Aksi untuk Admin --}}
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <div>
            <a href="{{ route('admin.tkw.showrt', ['rt' => $item->rt]) }}" class="btn btn-sm btn-outline-secondary shadow-sm">
                <i class="fas fa-arrow-left fa-sm"></i> Kembali ke Daftar RT {{ str_pad($item->rt, 2, '0', STR_PAD_LEFT) }}
            </a>
        </div>
    </div>

    {{-- BARIS UTAMA: Mengadopsi tata letak dari halaman user --}}
    <div class="row d-flex align-items-stretch">
        ...
        <div class="col-lg-7 mb-4 mb-lg-0 d-flex">
            <div class="card shadow-sm card-detail-section h-100 w-100">
                <div class="card-header py-3 d-flex justify-content-between align-items-center">
                    ...
                    <small class="text-muted">{{ \Carbon\Carbon::parse($item->tgl_pembuatan)->isoFormat('dddd, D MMMM GGGG') }}</small>
                </div>
                <div class="card-body p-3">
                    <dl class="row info-list-condensed mb-0">
                        <dt class="col-sm-5 col-lg-4">Nama Pendata</dt><dd class="col-sm-7 col-lg-8">{{ $item->nama_pendata }}</dd>
                        <dt class="col-sm-5 col-lg-4">Nama Responden</dt><dd class="col-sm-7 col-lg-8">{{ $item->nama_responden }}</dd>
                        <div class="col-12"><hr class="my-2"></div>
                        <dt class="col-sm-5 col-lg-4">Provinsi</dt><dd class="col-sm-7 col-lg-8">{{ $item->provinsi }}</dd>
                        <dt class="col-sm-5 col-lg-4">Kota / Kab</dt><dd class="col-sm-7 col-lg-8">{{ $item->kabupaten }}</dd>
                        <dt class="col-sm-5 col-lg-4">Kecamatan</dt><dd class="col-sm-7 col-lg-8">{{ $item->kecamatan }}</dd>
                        <dt class="col-sm-5 col-lg-4">Desa / Kelurahan</dt><dd class="col-sm-7 col-lg-8">{{ $item->desa }}</dd>
                        <dt class="col-sm-5 col-lg-4">RT / RW</dt><dd class="col-sm-7 col-lg-8">{{ str_pad($item->rt, 2, '0', STR_PAD_LEFT) }} / {{ str_pad($item->rw, 2, '0', STR_PAD_LEFT) }}</dd>
                    </dl>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-5 d-flex flex-column">
            <div class="card shadow-sm mb-4 card-recap">
                <div class="card-header py-3"><h6 class="m-0 font-weight-bold card-title">Ringkasan Rumah Tangga</h6></div>
                <div class="card-body">
                    <div class="summary-item"><span class="label"><i class="fas fa-users fa-fw mr-2 text-gray-400"></i>Total Anggota</span><span class="value">{{ $item->jart }}</span></div>
                    <div class="summary-item"><span class="label"><i class="fas fa-briefcase fa-fw mr-2 text-gray-400"></i>Anggota Bekerja</span><span class="value">{{ $item->jart_ab }}</span></div>
                    <div class="summary-item"><span class="label"><i class="fas fa-home fa-fw mr-2 text-gray-400"></i>Anggota Tidak Bekerja</span><span class="value">{{ $item->jart_tb }}</span></div>
                    <div class="summary-item"><span class="label"><i class="fas fa-user-graduate fa-fw mr-2 text-gray-400"></i>Anggota Masih Sekolah</span><span class="value">{{ $item->jart_ms }}</span></div>
                </div>
            </div>
            
            <div class="card shadow-sm flex-grow-1">
                <div class="card-header py-3"><h6 class="m-0 font-weight-bold card-title">Tindakan</h6></div>
                <div class="card-body d-flex flex-column">
                    <p class="text-gray-700">Tinjau dan proses pengajuan ini, atau unduh data dalam format Excel.</p>
                    <div class="mt-auto">
                        <a href="{{ route('admin.tkw.exportExcel', $item->id) }}" class="btn btn-success btn-icon-split mb-2">
                            <span class="icon text-white-50"><i class="fas fa-download"></i></span>
                            <span class="text">Export ke Excel</span>
                        </a>
                        <a href="{{ route('admin.tkw.show', $item->id) }}" class="btn btn-primary btn-icon-split">
                            <span class="icon text-white-50"><i class="fas fa-stamp"></i></span>
                            <span class="text">Proses Verifikasi Sekarang</span>
                        </a>
                    </div>
                </div>
            </div>
