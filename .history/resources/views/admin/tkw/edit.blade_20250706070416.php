...
@section('title', 'Edit & Verifikasi Pengajuan Ke-' . $item->user_sequence_number)

@push('styles')
<style>
    /* Style yang sudah kita sempurnakan sebelumnya */
    .card-body { padding: 1.25rem; }
    .card-header { padding: 0.75rem 1.25rem; }
    .sub-header-info { font-size: 0.9rem; font-weight: bold; color: #4e73df; margin-top: 1.5rem; margin-bottom: 0.25rem; padding-bottom: 0.5rem; border-bottom: 2px solid #4e73df; display: inline-block; }
...
@section('content')
{{-- Form utama yang akan mengirim semua data (editan & verifikasi) --}}
<form action="{{ route('admin.tkw.update', $item->id) }}" method="POST" enctype="multipart/form-data" id="main-form">
    @csrf
    @method('PUT')
    ...
    <div class="row">
        <div class="col-lg-7">
            <div class="card shadow-sm mb-4">
                <div class="card-header py-3"><h6 class="m-0 font-weight-bold text-primary">Data Pengajuan</h6></div>
                <div class="card-body py-3 px-4">
                    
                    <h6 class="sub-header-info">Informasi Pengajuan:</h6>
                    <div class="row mt-2">
                        <div class="col-md-6 mb-3"><label for="nama_pendata">Nama Pendata</label><input type="text" name="nama_pendata" id="nama_pendata" value="{{ old('nama_pendata', $item->nama_pendata) }}" class="form-control @error('nama_pendata') is-invalid @enderror">@error('nama_pendata')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                        <div class="col-md-6 mb-3"><label for="nama_responden">Nama Responden</label><input type="text" name="nama_responden" id="nama_responden" value="{{ old('nama_responden', $item->nama_responden) }}" class="form-control @error('nama_responden') is-invalid @enderror">@error('nama_responden')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                    </div>
                    
                    <h6 class="sub-header-info">Lokasi:</h6>
                    <div class="row mt-2">
                        <div class="col-md-6 mb-3"><label for="provinsi">Provinsi</label><input type="text" name="provinsi" id="provinsi" value="{{ old('provinsi', $item->provinsi) }}" class="form-control @error('provinsi') is-invalid @enderror"></div>
                        <div class="col-md-6 mb-3"><label for="kabupaten">Kabupaten/Kota</label><input type="text" name="kabupaten" id="kabupaten" value="{{ old('kabupaten', $item->kabupaten) }}" class="form-control @error('kabupaten') is-invalid @enderror"></div>
                        <div class="col-md-6 mb-3"><label for="kecamatan">Kecamatan</label><input type="text" name="kecamatan" id="kecamatan" value="{{ old('kecamatan', $item->kecamatan) }}" class="form-control @error('kecamatan') is-invalid @enderror"></div>
                        <div class="col-md-6 mb-3"><label for="desa">Desa/Kelurahan</label><input type="text" name="desa" id="desa" value="{{ old('desa', $item->desa) }}" class="form-control @error('desa') is-invalid @enderror"></div>
                        <div class="col-md-6 mb-3"><label for="rt">RT</label><input type="number" name="rt" id="rt" value="{{ old('rt', $item->rt) }}" class="form-control @error('rt') is-invalid @enderror" min="0"></div>
                        <div class="col-md-6 mb-3"><label for="rw">RW</label><input type="number" name="rw" id="rw" value="{{ old('rw', $item->rw) }}" class="form-control @error('rw') is-invalid @enderror" min="0"></div>
                    </div>
        ...
        <div class="col-lg-5 mb-4">
            <div class="card shadow-sm h-100">
                ...
                <div class="card-body">
                    {{-- Form verifikasi diletakkan di sini --}}
                    <div class="mb-3"><label class="form-label">Pilih Status Verifikasi <span class="text-danger">*</span></label><div class="d-flex"><div class="custom-control custom-radio mr-4"><input type="radio" id="status_validated" name="status" value="validated" class="custom-control-input" @if(old('status', $item->status_validasi) == 'validated' || $item->status_validasi == 'pending') checked @endif><label class="custom-control-label" for="status_validated">Setujui</label></div><div class="custom-control custom-radio"><input type="radio" id="status_rejected" name="status" value="rejected" class="custom-control-input" @if(old('status') == 'rejected') checked @endif><label class="custom-control-label" for="status_rejected">Tolak</label></div></div></div>
                    <div class="mb-3"><label for="admin_tgl_validasi" class="form-label">Tanggal Verifikasi <span class="text-danger">*</span></label><input type="date" name="admin_tgl_validasi" id="admin_tgl_validasi" value="{{ old('admin_tgl_validasi', optional($item->admin_tgl_validasi)->format('Y-m-d') ?? now()->toDateString()) }}" class="form-control @error('admin_tgl_validasi') is-invalid @enderror">@error('admin_tgl_validasi')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                    <div class="mb-3"><label for="admin_nama_kepaladusun" class="form-label">Nama Verifikator <span class="text-danger">*</span></label><input type="text" name="admin_nama_kepaladusun" id="admin_nama_kepaladusun" value="{{ old('admin_nama_kepaladusun', $item->admin_nama_kepaladusun ?? Auth::user()->name) }}" class="form-control @error('admin_nama_kepaladusun') is-invalid @enderror">@error('admin_nama_kepaladusun')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                    <div class="mb-3"><label for="admin_ttd_pendata" class="form-label">Upload TTD Verifikator</label><input type="file" name="admin_ttd_pendata" id="admin_ttd_pendata" class="form-control-file @error('admin_ttd_pendata') is-invalid @enderror"><small class="text-muted d-block">Kosongkan jika tidak ingin mengubah TTD.</small><div id="signature-preview" class="mt-2"></div>@error('admin_ttd_pendata')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror</div>
                    <div class="mb-3"><label for="admin_catatan" class="form-label">Komentar / Alasan Penolakan</label><textarea name="admin_catatan" id="admin_catatan" rows="4" class="form-control @error('admin_catatan') is-invalid @enderror" placeholder="Wajib diisi jika menolak pengajuan...">{{ old('admin_catatan', $item->admin_catatan) }}</textarea>@error('admin_catatan')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror</div>
                </div>
            </div>
        </div>
    </div>

    {{-- Bagian Anggota Keluarga (full-width) --}}
    <div class="card shadow-sm mb-4">
        ...
        <div id="members-container" class="w-100 card-body">
            @foreach (old('nama', $item->anggotaKeluarga) as $index => $data)
                ...
                <div class="member-form card mb-3">
                     {{-- ... isi form anggota keluarga (sama seperti sebelumnya) ... --}}
                </div>
            @endforeach
        </div>
        <div class="card-footer bg-light d-flex justify-content-between align-items-center">
            <small class="text-muted pr-3">Tambahkan atau hapus anggota keluarga sesuai kebutuhan.</small>
    ...
    <div class="w-100 d-flex justify-content-between my-4">
        <div><a href="{{ route('admin.tkw.show', $item->id) }}" class="btn btn-secondary">Batal</a></div>
        <button type="submit" class="btn btn-primary btn-lg"><i class="fas fa-save mr-2"></i>Simpan Perubahan & Proses</button>
    </div>
</form>
@endsection
...
@push('scripts')
<script>
    // ... (Gabungkan semua JavaScript yang relevan di sini)
</script>
@endpush
