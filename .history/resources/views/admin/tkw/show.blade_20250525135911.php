...
@section('title', 'Detail Validasi Rumah Tangga')

@section('content')
<div class="container-fluid">
    {{-- Judul Halaman dan Tombol Kembali --}}
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 text-gray-800 mb-0">Validasi Rumah Tangga #{{ $item->id }} (Responden: {{ $item->nama_responden }})</h1>
        <a href="{{ route('admin.validation.tkw.index') }}" class="btn btn-sm btn-outline-secondary shadow-sm"> {{-- Sesuaikan nama route jika perlu --}}
    ...
    <div class="row">
        ...
        <div class="col-lg-7 mb-4"> {{-- Lebar kolom sedikit diubah untuk proporsi --}}
            <div class="card shadow-sm h-100">
                <div class="card-header bg-primary text-white py-3">
                    <h6 class="m-0 font-weight-bold">Data Rumah Tangga</h6>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <p class="mb-2"><strong>RT/RW:</strong><br>{{ $item->rt }}/{{ $item->rw }}</p>
                        </div>
                        <div class="col-md-6">
                            ...
                            <p class="mb-2"><strong>Status Saat Ini:</strong><br>
                                @php
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <p class="mb-2"><strong>Jml. Anggota RT:</strong><br>{{ $item->jart }}</p>
                            <p class="mb-2"><strong>Jml. ART Bekerja:</strong><br>{{ $item->jart_ab }}</p>
                            <p class="mb-0"><strong>Pendapatan RT/Bulan:</strong><br>{{ $item->jpr2rtp_text ?? $item->jpr2rtp }}</p>
                        </div>
                        @if($item->ttd_pendata)
                        <div class="col-md-6">
        ...
        <div class="col-lg-5 mb-4"> {{-- Lebar kolom sedikit diubah --}}
            <div class="card shadow-sm h-100">
                <div class="card-header bg-success text-white py-3">
                    <h6 class="m-0 font-weight-bold">Form Tindakan Validasi</h6>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.validation.tkw.approve', ['id' => $item->id]) }}" method="POST" enctype="multipart/form-data"> {{-- Sesuaikan nama route --}}
                        @csrf
                        <div class="mb-3">
                        ...
                        @else
                            <div class="alert alert-info mt-3 text-center">Tindakan validasi sudah dilakukan. <br> Status: <strong>{{ $item->status_validasi_text }}</strong></div>
                        @endif
                    </form>
                </div>
            </div>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                {{-- Pastikan route 'admin.validation.tkw.reject' benar --}}
                <form action="{{ route('admin.validation.tkw.reject', ['id' => $item->id]) }}" method="POST" style="display: inline;">
                    @csrf
...
{{-- Script Preview Gambar (tetap sama atau bisa dipindah) --}}
@push('scripts')
<script>
document.addEventListener('DOMContentLoaded', function () {
    ...
    if(inputTtd && previewTtd) { // Cek apakah elemen ada
        inputTtd.addEventListener('change', function () {
            const file = this.files[0];
            // Bersihkan preview sebelum menambahkan yang baru, kecuali jika ada TTD lama
            if (!previewTtd.querySelector('img[src*="storage"]')) { // Cek jika bukan gambar dari storage
            ...
            if (file) {
                ...
                reader.onload = function (e) {
                    // Hapus preview lama jika ada dan bukan dari storage
                    const existingDynamicPreview = previewTtd.querySelector('img:not([src*="storage"])');
                    if(existingDynamicPreview) existingDynamicPreview.remove();
