...
<form action="{{ route('admin.tkw.update', $item->id) }}" method="POST" enctype="multipart/form-data" id="main-form">
    ...
    <fieldset @if($item->status_validasi === 'rejected') disabled @endif>
...
const dbTglValidasi = tglValidasiInput?.value || '';
const dbNamaVerifikator = namaVerifikatorInput?.value || '';
const dbSignatureHTML = signaturePreview?.innerHTML || '';
    ...
    // SIMPAN NILAI AWAL AGAR TIDAK HILANG SAAT TOGGLE
    const initialTglValidasi = tglValidasiInput?.value || '';
    const initialNamaVerifikator = namaVerifikatorInput?.value || '';
    const initialSignatureHTML = signaturePreview?.innerHTML || '';
    ...
    function updateFormState() {
        ...
        if (radioValidated.checked) {
            ...
            // KEMBALIKAN NILAI DARI DB JIKA PERNAH DIKOSONGKAN
            if (!tglValidasiInput.value) {
                tglValidasiInput.value = initialTglValidasi || today;
            }
            if (!namaVerifikatorInput.value) {
                namaVerifikatorInput.value = initialNamaVerifikator;
            }
            if (signaturePreview.innerHTML.trim() === '') {
                signaturePreview.innerHTML = initialSignatureHTML;
            }
            ...
            // KOSONGKAN FIELD TANPA HILANG PERMANEN
